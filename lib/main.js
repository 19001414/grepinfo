...
$(function () {
    ...
    if (searchValue) $('.searchvalue[data-type="buffer"]').val(searchValue);
    ...
    $('.searchvalue').on('input', function () {
        ...
        value = $input.val();
        if (type !== 'buffer') result[type] = value;else searchValue = value;
    });

    $('.submit').on('click', function () {
        var url, $navbar, $pageText;
...
function addnewheader($parent) {
    var $span;
    $span = $('<span class="configinput">' + '<input type="text" placeholder="key"/><input type="text" placeholder="value" />' + '<span class="myadd" title="添加">+</span>' + '<span class="myclose" title="删除">&times;</span>' + '</span>');
    $parent.append($span);
...
function searchData(searchValue, searchMethod, searchType) {
    ...
    return $.ajax({
        url: 'http://10.40.6.82:29900/iLoc',
        method: 'POST',
